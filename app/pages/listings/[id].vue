  ...
  <div class="min-h-screen bg-[#FAFAFA]">
    ...
    <div v-else-if="l" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      <nav class="flex items-center gap-2 text-xs text-gray-400 mb-4">
        ...
        <span class="text-gray-600">{{ l.location || 'Details' }}</span>
      </nav>
        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <div class="bg-white rounded-2xl shadow-sm p-6">
            <div class="flex flex-wrap gap-2 mb-4">
              ...
              <span v-if="l.other_features?.is_furnished === true" class="tag tag-green">Furnished</span>
              <span v-if="l.other_features?.is_furnished === false" class="tag">Unfurnished</span>
              <span v-if="l.other_features?.is_new" class="tag tag-violet">New</span>
            </div>
            ...
            <div class="mt-5 pt-5 border-t border-gray-100">
              <p class="text-3xl font-bold text-[#0a0a0a]">
                ...
                <span class="text-base font-normal text-gray-400">
                  {{ l.property_typology === 'rent' ? '/month' : '' }}
                </span>
              </p>
            </div>
...
const details = computed(() => {
  ...
  const other = l.value.other_features
  const extra = l.value.extra_data

  if (interior?.rooms) d['Rooms'] = interior.rooms
  if (interior?.surface_m2) d['Surface'] = `${interior.surface_m2} m²`
  if (interior?.floor !== undefined) d['Floor'] = interior.floor
  if (interior?.heating) d['Heating'] = interior.heating
  if (exterior?.has_elevator !== undefined) d['Elevator'] = exterior.has_elevator ? 'Yes' : 'No'
  if (exterior?.balconies) d['Balconies'] = exterior.balconies
  if (exterior?.terraces) d['Terraces'] = exterior.terraces
  if (exterior?.parking_spots) d['Parking'] = exterior.parking_spots
  if (l.value.price_per_m2) d['Price/m²'] = `${l.value.price_per_m2} ${l.value.currency}`
  if (extra?.charges) d['Charges'] = `${extra.charges} ${l.value.currency}`
  if (extra?.agency_fee) d['Agency fee'] = `${extra.agency_fee} ${l.value.currency}`
  if (extra?.reference) d['Reference'] = extra.reference
  if (extra?.postal_code) d['Postal code'] = extra.postal_code
